...
## Итерация по коллекциям: снапшот из коробки

`for x in list` / `for k in map` / `for k, v in map` — цикл видит коллекцию на
момент входа (снапшот в пулируемом буфере файбера): менять её в теле безопасно.
Для Map удалённые ключи молча пропускаются, `v` читается живым (`map[k]`).
Снапшот переживает `wait` внутри цикла и сейв/лоад; буфер освобождается на
выходе и гарантированно при любой смерти файбера — путей утечки нет.

```
for k, v in inventory {                    // v — живое значение (map[k])
    if (v <= 0) { inventory.Remove(k); }   // мутация в цикле безопасна
}
