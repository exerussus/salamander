...
host.Struct<Damage>("Damage", "Урон по типам.")
    .Field<float>("pierce").Field<float>("slash").Field<float>("blunt")
    .Field<float>("fire")
...
Сравнивать структуры нельзя (`==` шло бы по хэндлу, и две одинаковые оказались
бы неравны — сравнивайте нужные поля) и ключом `Map` они быть не могут.
...
- **Структуры хоста**: `new Damage(slash: 21)` — именованный неизменяемый набор
  полей с дефолтами; тип объявляет игра, читает обратно через `ReadStruct<T>`.
