...
```
int hp = Math.Clamp(hp - dmg, 0, maxHp);          // int, int, int → int
float k = Math.Lerp(0.2, 1.0, progress);          // t зажат в [0, 1], как Mathf.Lerp
int stacks = Math.Floor(power / 10.0);             // Floor/Ceil/Round — всегда int
float r = Math.Sqrt(dx * dx + dy * dy);
float full = Math.PI * 2;                          // константа, без скобок
```

`Min`, `Max`, `Clamp`, `Abs`, `Sign`, `Floor`, `Ceil`, `Round`, `Sqrt`, `Pow`,
`Lerp` и `Math.PI`.

- **Тип результата — общий тип аргументов** (как у арифметики: int → float →
  double): `Math.Min(3, 7)` — int, `Math.Min(3, 1.5)` — float, в `int` без
  явного `Floor`/`Round` не кладётся. `Sqrt`/`Pow`/`Lerp` — не ниже float,
  `Sign`/`Floor`/`Ceil`/`Round` — всегда int (как `Mathf.FloorToInt`).
- **`Round` — половина от нуля**: 2.5 → 3, −2.5 → −3 (не банковское округление).
- **Никаких тихих NaN и переполнений.** Корень из отрицательного, нечисловой
  или бесконечный результат `Pow`, `Floor`/`Round` вне диапазона int,
  `Abs(-2147483648)`, `Clamp` с min > max — ошибка скрипта с понятным
  текстом (гибнет только этот файбер), а не мусор, который всплывёт позже.
- `Math.PI` — float (как литерал `3.14`): в double расширится сам, в float не
  сузится. Сворачивается в литерал — вызова в байткоде нет.
- **Своё имя важнее.** Если игра зарегистрировала API, класс или енум `Math`
  или в скриптах есть `class Math` — работает он, а не встроенный. Старые
  проекты не ломаются.
- Реализация — те же `CallEngine`, что у `Engine`: чистые функции, сейв и
  отпечаток программы у скриптов без `Math` не меняются.

Ошибки: E0249 — у Math нет такого члена; E0250 — не то число аргументов;
E0251 — аргумент не число; `Math.PI()` — E0239, `Math.Min` без скобок — E0162.

### `++`, `--`, `%=`

```
hits++;  combo--;  ++wave;  stacks[i]++;  u.hp--;  C.count++;
turn %= 4;
```

- **Только стейтмент**, значения у него нет: `int y = x++;` и `y = ++x;` —
  ошибка E0248 («сначала `x++;`, потом используйте x»). Поэтому префиксная и
  постфиксная формы значат одно и то же, а вопрос «до или после» не возникает.
- Это ровно `x += 1` / `x -= 1`: работает везде, где работает `+=` (локаль, поле,
  поле подписки listener, поле класса через точку, элемент массива, записываемое
  свойство игры), с теми же проверками `readonly` и тем же расширением типов.
- Цель — число (int/float/double). `s++` для строки — E0247: иначе `+= 1` молча
  дописал бы к строке `"1"`.
- `%=` — как `%`: только `int %= int` (E0215).
- Как в C#, `--` слитно — один токен: `a--b` больше не читается как `a - (-b)`.
  С пробелом (`a - -b`) — по-прежнему.
...
protected override void ConfigureHost(HostRegistry registry)
{
    ...
    // у объекта бывают и свои методы — когда он и ЕСТЬ интерфейс:
    // в скрипте это basket.AddPerk("dummy_trainer")
    host.Class<PerkBasket>("PerkBasket", "Корзина перков, собираемая скриптом.")
...
## IDE скриптов в игре (Dsl.Ide)

`Dsl.Ide` — редактор скриптов на UI Toolkit, который работает прямо в
плеер-билде (моддинг-консоль, живая правка на тестовом стенде) и в редакторе
Unity (страница Nexus — та же IDE, другой хост). Внутри: дерево модулей и
вкладки, подсветка, автодополнение, подсказка параметров, hover и переход к
определению (общий с LSP сервис `Dsl.Tooling`), номера строк, поиск/замена,
...
(файл манифеста не нужен), модули — из папки бутстрапа на десктопе или из
памяти на WebGL/мобильных. Подробности и требования к платформам — в
`INTEGRATION.md`, раздел «IDE скриптов».
...
### Методы объекта: когда объект и есть интерфейс

Классу-сущности можно объявить не только свойства, но и методы — тогда вызов
пишется на самом значении:
...
{
    b.AddPerk("dummy_trainer");        // вместо Api.Npc.Perk(b, "dummy_trainer")
}
```

Приёмник уходит **нулевым аргументом** хостовой функции, поэтому `b.AddPerk(x)`
и `Api.Npc.Perk(b, x)` компилируются в один и тот же `CallHost` — побайтово
(это закреплено тестом). Разница не в цене, а в том, ГДЕ читатель ищет вызов,
и отсюда правило:

- **метод у класса** — когда объект существует ради вызовов: корзина, билдер,
  накопитель, который игра передаёт в событие. `Api.Npc.Perk(b, ...)` для такого
  объекта читается наизнанку;
- **API-класс** — когда объект это данные (`Unit`, `Item`). Иначе половина API
  ищется в списке API, а половина — только через переменную нужного типа.

Имя у класса одно на всех: свойством и методом одновременно оно быть не может
(регистрация бросает). Вызов метода на пустой ссылке — скриптовая ошибка с
внятным текстом, а не тихий вызов с `this == null`; свойство, вызванное со
скобками, — **E0240** («читайте его без скобок»).
...
### Слои: `before` / `after` / `replace` и `base()`

Переопределение заменяет член целиком, а моду чаще нужно ДОПОЛНИТЬ механику:
сделать что-то до или после, не копируя чужое тело. Для этого у события,
функции и `action` есть слово перед объявлением:

```
// база
spell fireball { event OnCast(Unit c, float p) { Api.Burn(c, p); } }

// мод
spell fireball {
    before event OnCast(Unit c, float p) { Fx.Charge(c); }     // до ядра
    after  event OnCast(Unit c, float p) { casts = casts + 1; } // после ядра
}
```

Все версии одного члена — это одна **цепочка** в порядке загрузки (модуль →
файл → блок). Что делает каждая запись с тем, что объявлено раньше:

| Запись | Что происходит |
|---|---|
| `before event X {…}` | слой ДО ядра |
| `after event X {…}` | слой ПОСЛЕ ядра |
| `event X {…}` | меняется только ядро, слои остаются; внутри можно `base(...)` |
| `event X;` / `pass;` | ядро убито, слои остаются |
| `replace event X {…}` | стёрто всё раньше — и ядро, и слои; это новое ядро |
| `replace event X;` | член заглушён целиком, со слоями модов, загруженных раньше |

Правила, которые из этого следуют:

- **Слои идут в порядке загрузки**: сначала все `before`, потом ядро (последняя
  обычная версия или `replace`), потом все `after`. Слой раннего мода
  выполняется раньше слоя позднего — и в `before`, и в `after`.
- **Одна цепочка — один файбер.** `wait` в `before` задерживает ядро; `return` в
  слое завершает только слой, ядро всё равно выполнится. Нужен параллельный
  запуск — `spawn` внутри слоя.
- **`replace` стирает только то, что загружено раньше.** Моды, загруженные
  позже, снова кладут слои поверх: ранний мод не может запретить что-то
  позднему. Порядок задают зависимости — чтобы гарантированно стереть мод X,
  укажите его в `dependencies` (компилятор сортирует модули по зависимостям,
  независимые — по имени).
- **Внутри одного блока `replace` (и обычная версия) применяется первым**, как бы
  ни стояли строки: мод-переделка может в одном блоке написать `replace` и свой
  `after`.
- **`base(...)`** в обычном переопределении вызывает предыдущую версию того же
  члена — можно с другими аргументами и с её результатом:
  `func Dmg(int x) -> int { return base(x * 2) + 1; }`. В `before`/`after`
  (ядро и так выполнится) и в `replace` (под ним ничего нет) это ошибка.
  Своя функция с именем `base` важнее слова — старые скрипты не ломаются.
- **Функция с результатом**: слою результат объявлять не нужно
  (`after func Dmg(int x) { ... }`) — вызывающий получит результат ядра.
  Одни слои без ядра у функции с результатом — E0246.
- **Сигнатуры.** Когда между версиями есть слои или разные модули, цепочка
  вызывает каждую версию одними аргументами, поэтому версии, до которых она
  может дойти, обязаны совпадать по сигнатуре (E0242; виновата версия,
  разошедшаяся позже, — при карантине по файлу выпадает мод, а не база). Ядра
  модуля-владельца ниже его последнего ядра не сверяются: до них доходит только
  `base()`, и его проверяет сам вызов.
- **Поля слоёв не имеют** — поздний блок просто перезаписывает значение (E0241
  на `after int x;`).
- `before`/`after`/`replace`/`base` — **контекстные** слова, как `readonly`:
  поле, локаль или функция с таким именем компилируются как раньше.

**Выключенный модуль прозрачен.** Сущностью владеет модуль её первого блока:
выключили его (`Engine.DisableModule`) — молчит вся сущность, как и раньше у
триггеров и listener. Версии ДРУГИХ модулей проверяют свой модуль на каждом
вызове: выключенного модуля как будто нет — его слои пропускаются, его обычное
переопределение уступает предыдущей версии, его `replace` ничего не стирает,
`base()` проходит мимо его версии. Модуль вызывающей версии для её `base()`
считается включённым: файбер, уснувший в версии до `DisableModule`, доработает
так, как начал.

Что поменялось для уже написанных модов:

- выключение мода, переопределившего событие архетипа, раньше глушило событие
  совсем — теперь возвращается версия базы; у триггера, listener и функций патч
  выключенного мода раньше продолжал работать — теперь тоже возвращается база;
- член, который мод ДОБАВИЛ чужой сущности (обработчик в триггер базы), раньше
  работал и при выключенном моде — теперь молчит, как и положено;
- архетип теперь выключается модулем первого блока (раньше — модулем
  победившей версии);
- переопределение из другого модуля с ДРУГОЙ сигнатурой раньше молча
  выигрывало, теперь это E0242: выключив мод, цепочка позвала бы базу его
  аргументами.

**Цена.** Если все версии члена — в модуле-владельце и слоёв нет, байткод
ровно прежний: вход — сама последняя версия. Иначе компилятор собирает
синтетическую функцию-цепочку: слои → первое включённое ядро → слои, всё
обычными вызовами в том же файбере (одна проверка модуля на версию мода,
новых механизмов в VM нет). Это +1 кадр на вызов (и ещё +1 у `base()` через
версию мода) — рекурсия в таком члене упрётся в `MaxCallDepth` примерно вдвое
раньше. Сейвы видят кадры цепочки как любые другие; в стектрейсе версии
подписаны: `fireball.OnCast [after]`, `fireball.OnCast [цепочка]`.

Тело версии проверяется в контексте СВОЕГО модуля: слой мода видит классы мода
и его зависимостей и подчиняется его режиму исполнения. (Раньше все версии
мерж-сущности проверялись в контексте модуля первого блока — патч мода не
видел собственных классов.)

Ошибки: E0241 — слово перед полем; E0242 — сигнатура версии расходится (при
слоях, гейте модулей или через `base()`); E0243 — `base()` в слое; E0244 —
`base()` в `replace`; E0245 — `base()` без более ранней версии или вне тела
члена; E0246 — у функции с результатом одни слои и нет ядра.

В редакторе наведение на имя члена в объявлении (или на `before`/`after`/
`replace`/`base`) показывает все его версии по файлам и что выполнится в итоге.

## Карантин: сбойный мод не гасит остальное

Инструменты (DslCheck, LSP, IDE) компилируют без карантина: любая ошибка —
`Program == null`, модер видит все свои ошибки. Игра собирает с карантином —
сломанное исключается, остальное работает:

```csharp
var r = ScriptCompiler.Compile(registry, apiVersion, modules, quarantineBrokenModules: true);
// r.Excluded      — исключённые модули и почему
// r.ExcludedFiles — исключённые файлы (режим File), их ошибки — в r.Diagnostics
```

Чем платит ошибка в ИСХОДНИКЕ, решает хост — это свойство того, как у игры
разложен контент, а не конкретного вызова:

```csharp
host.Quarantine(QuarantineScope.File);   // то же: registry.Quarantine = QuarantineScope.File
```

| Режим | Ошибка в исходнике | Беды без файла (E0300–E0306) |
|---|---|---|
| `Module` (по умолчанию) | модуль исключается вместе с зависимыми | модуль с зависимыми |
| `File` | исключается только файл; модуль и зависимые собираются | модуль с зависимыми |

`File` нужен при правиле «один файл — одна сущность»: опечатка в
`core.items/content/coin.sal` стоит монеты, а не всех предметов, всех жителей,
которые от предметов зависят, и всего, что зависит от жителей. Как идёт сборка:

- синтаксическая ошибка останавливает проход до чекера, поэтому первыми
  выпадают файлы с ошибками парсера;
- следующим проходом чекер находит семантические ошибки — в том числе у файлов,
  которые звали функцию, класс или enum из выкинутого; они выпадают тоже;
- и так, пока сборка не сойдётся. Каждый проход выкидывает хотя бы один файл
  или модуль, поэтому цикл конечен; проходов больше одного только на пути ошибки.

Что стоит знать:

- ошибки выкинутых файлов не теряются — они в `Diagnostics` с кодом, файлом и
  строкой, даже когда `Success`;
- модуль, у которого выпали ВСЕ файлы, жив, и зависимые собираются; W0300
  («модуль без исходников») он не получает — исходники были, их снял карантин;
- ошибка без файла (`<unknown>`, переполнение E0221) решается по-старому: если
  рядом есть ошибки с файлами, сначала выпадают они, иначе — прежний путь;
- сломан патч в моде — выпадает только патч, сущность откатывается к базе;
- сломана база, а патч цел — патч становится единственным объявлением. Если у
  вида есть обязательные (`required`) константы, которых в патче нет, патч
  следующим проходом получает E0223 и выпадает тоже: сущности нет, а не
  «полусущность». Ровно так же уходит и сущность, у которой в патче нет ни одного
  события вида (E0199).
...
Наведение показывает всё, что игра описала: методы и их параметры (у API-классов
и у самих объектов), события (свои и у вида), константы вида (тип, дефолт,
«обязательная»), поля структур, свойства сущностей, элементы енумов, константы
API и узлы составных имён (`Api.`, `Api.PartsCatalog.`) — с тем самым
`doc`/`summary`, который задал хост. Константы API при этом и красятся отдельно от свойств: у них нет
...
- **Встроенный `Math`** (Min/Max/Clamp/Abs/Sign/Floor/Ceil/Round/Sqrt/Pow/Lerp,
  `Math.PI`) с выводом типа результата и ошибками вместо NaN; `x++`/`x--`/`%=`
  как стейтменты.
...
- **Мерж-переопределение**: поздние блоки патчат события/поля/функции любой
  декларации — моды меняют механики базы без правки её файлов. Слои
  `before`/`after` дополняют член, не копируя его, `replace` стирает всё раньше,
  `base(...)` зовёт предыдущую версию; выключенный модуль прозрачен.
...
- **Хост-API**: fluent (`Class/Prop/Fn/Act/Event/Enum/Struct/Archetype`, методы
  объекта — `basket.AddPerk("x")` с тем же байткодом, что у статической формы) и атрибуты
...
- **Модули**: манифесты, зависимости (топологический порядок),
  ...
  **карантин**: в игре сбойный модуль исключается вместе с зависимыми, а не
  ...
  с `host.Quarantine(QuarantineScope.File)` ошибка в исходнике стоит только
  файла (`CompilationResult.ExcludedFiles`) — см. «Карантин».
