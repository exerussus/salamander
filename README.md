...
- **Сборщика коллекций в v1 нет, и это важнее, чем звучит.** Слот коллекции
  освобождается только при перезагрузке программы, поэтому вечной становится не
  только долгоживущая, но и любая ВРЕМЕННАЯ коллекция: `var tmp = new List<int>();`
  в обработчике, который срабатывает каждый кадр, течёт со скоростью кадра.
  Заводите коллекции в статических полях один раз и переиспользуйте
  (`list.Clear()`), а не создавайте на горячем пути. Пока сборщика нет, движок
  бьёт по рукам лимитом `Collections.MaxLiveCollections` (внятная ошибка вместо
  тихого роста до OutOfMemory) — счётчик живых коллекций виден в `GetStats()`.
...
| `Collections.MaxLiveCollections` | 1 048 576 | утечка коллекций (см. выше) |
