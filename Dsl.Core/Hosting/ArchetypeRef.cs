...
namespace Dsl.Hosting
{
    ...
    public sealed class ArchetypeBuilder
    {
        ...
        /// <summary>
        /// Сущности этого вида вправе не реализовывать НИ ОДНОГО события (E0199
        /// снимается). Два случая: вид-конфиг (описание атрибута, рецепт — данные
        /// без механики) и вид, у которого поведение по умолчанию живёт в игре, а
        /// блок только уточняет числа.
        ///
        /// По умолчанию выключено, и это не формальность: E0199 ловит «блок
        /// написан и ничего не делает» — недописанную после переименования
        /// механику, событие, чьё имя уехало в поле. Включайте там, где данные
        /// без механики — норма, а не забытая работа.
        ///
        /// Виду, у которого хост не объявил событий вовсе, флаг не нужен: с него
        /// требование снимается само (иначе E0199 требовала бы событие, а E0217
        /// запрещала любое его имя).
        /// </summary>
        public ArchetypeBuilder EventsOptional(bool optional = true)
        {
            _host.Registry.SetArchetypeEventsOptional(KindId, optional);
            return this;
        }
